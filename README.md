...
AgentSos is a production-ready SaaS boilerplate built with Elixir, Phoenix LiveView, and the Ash Framework. It provides multi-tenant workspaces, AI agent management (Anthropic + OpenAI), Stripe billing, team collaboration, and a real-time dashboard — all wired together and ready to deploy.
...
```bash
# 1. Clone and install
git clone <repo-url> agentsos && cd agentsos
mix setup
...
```
lib/agent_sos/              # Domain layer (Ash resources)
...
AgentSos auto-generates REST and GraphQL APIs from Ash resources:
...
mix agent_sos.reset         # Reset database
mix agent_sos.seed          # Seed demo data
mix agent_sos.seed_plans    # Seed billing plans
```
